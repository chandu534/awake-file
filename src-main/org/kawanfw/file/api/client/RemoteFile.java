 ...
 * <li>{@code File.compareTo(File)} and {@code File.renameTo(File)} methods
 * take a {@code RemoteFile} parameter in this class.</li>
 ...
 * not ported.</li>
 * <li>The Java 7+ {@code File.toPath()} method is not ported as this Awake FILE
 * version does not support remote {@code Path} objects.</li>
 * </ul>
 * <p>
 * Note that the <i>real</i> pathname used on host for {@code File} method
 * execution depends on the Awake FILE configuration of
 * {@link FileConfigurator#getServerRoot()} and
 * {@link FileConfigurator#useOneRootPerUsername()}. This follow the same
 * principle of FTP server mechanisms.
 * <p>
 * Example:
 * <p>
 * If {@code FileConfigurator.getServerRoot()} returns {@code /home/} and
 * {@code FileConfigurator.useOneRootPerUsername()} returns {@code true} and
 * client username is {@code "mike"} : <br>
 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
 ...
 * Commons IO 2.4</a> are included in this software and the <a href=
 ...
 * <li>{@link RemoteException}:  an exception has been thrown on the server side.</li>
 * </ul>
 * <br>
 * Note: the user rights are the rights of
 * the servlet container when accessing remote files.
 * <h3>Example of RemoteFile usage</h3>
 * <blockquote>
 * 
 * <pre>
...
public class RemoteFile {
	...
	try {
	    ...
	    String result = remoteFileExecutor.fileMethodOneReturn(
		    this.pathname, "compareTo", pathname);
	...
	try {
	    String result = remoteFileExecutor.fileMethodOneReturn(
		    this.pathname, "getCanonicalFile");
	    return new RemoteFile(remoteSession, result);
	} catch (RemoteException remoteexception) {
	    Throwable cause = remoteexception.getCause();
	    if (cause != null && cause instanceof IOException) {
	...
	try {
	    String result = remoteFileExecutor.fileMethodOneReturn(
		    this.pathname, "getCanonicalPath");
	    return result;
	} catch (RemoteException remoteexception) {
	    Throwable cause = remoteexception.getCause();
	    if (cause != null && cause instanceof IOException) {
     ...
     * when the <code>{@link
 FilenameFilter#accept}</code> method of the filter is invoked on
     * this abstract pathname and the name of a file or directory in the
	...
	try {
	    ...
	    filenames = remoteFileExecutor.list(
		    this.pathname, filter, callerClassName);
	...
	try {
	    RemoteFile[] remoteFiles = remoteFileExecutor.listFiles(
	...
	try {
	    ...
	    remoteFiles = remoteFileExecutor.listFiles(
		    this.pathname, filter, null, callerClassName);
	...
	try {
	    ...
	    remoteFiles = remoteFileExecutor.listFiles(
		    this.pathname, null, filter, callerClassName);
